...
class ClassifierRuntime:
    ...
    def predict(self, image: np.ndarray) -> dict[str, Any]:
        ...
        # Final decision stays aligned with the filtered stable result so
        # webpage and serial reflect exactly the same control judgment.
        control_state = self.stable_state
        control_score = clutter_smooth if control_state == "clutter" else 1.0 - clutter_smooth
...
class SerialSender:
    ...
    def build_payload(self, snapshot: dict[str, Any]) -> bytes:
        clutter_flag = 1 if snapshot.get("state") == "clutter" else 0
        ...
        state_name = str(snapshot.get("state", "unknown")).upper()
    ...
    def loop(self) -> None:
        ...
        while True:
            try:
                try:
                    snapshot = self.tx_queue.get(timeout=0.10)
